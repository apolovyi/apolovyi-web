...
function generateCareerData() {
	...
	// Count entries by extracting each array section between its declaration and closing bracket
	const countInArray = (arrayName) => {
		const regex = new RegExp(`export const ${arrayName}[\\s\\S]*?\\n\\]`)
		const section = careerData.match(regex)
		return section ? (section[0].match(/\bid:\s*'/g) || []).length : 0
	}
	const stationCount = countInArray('stations')
	const certCount = countInArray('certifications')
	const eduCount = countInArray('education')

	const exportedFunctions = [
		...careerData.matchAll(/export\s+function\s+(\w+)\s*\(/g),
	].map((m) => m[1])
	const exportedTypes = [
		...careerData.matchAll(/export\s+(?:type|interface)\s+(\w+)/g),
	].map((m) => m[1])
