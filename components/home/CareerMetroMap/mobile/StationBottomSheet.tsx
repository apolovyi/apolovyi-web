...
export function StationBottomSheet({ activeStation }: StationBottomSheetProps) {
	const [isExpanded, setIsExpanded] = useState(false)
	const dictionary = useDictionary()
	const sheetRef = useRef<HTMLDivElement>(null)

	// Reset expanded state when station changes
	useEffect(() => {
		setIsExpanded(false)
	}, [activeStation])
				...
				<AnimatePresence>
					{isExpanded && (
						<motion.div
							...
							style={{ maxHeight: BOTTOM_SHEET_CONFIG.expandedHeight - 120 }}
						>
							{/* Metrics */}
							{metrics.length > 0 && (
