...
export default function LanguageDetector() {
	...
	logger.debug(`[LanguageDetector] Incoming request for path: ${pathname}`)
		...
		const setLanguageInCookie = () => {
			...
			if (!existingLang) {
				...
				if (i18n.locales.includes(currentLocale)) {
					logger.debug(`[LanguageDetector] set locale: ${currentLocale}`)
					...
					logger.debug(`[LanguageDetector] set locale by default: ${defaultLocale}`)
				}
			}
		}
