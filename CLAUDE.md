...
```bash
pnpm run dev              # Dev server (Turbopack, port 3100)
...
- **Dictionary system**: `dictionaries/*.json` validated with Zod, accessed via `getDictionary()`

Details: `.claude/docs/architecture.md`

### Deployment

Production on Netlify (static, auto-deploys from `main`). Analytics: TinyAnalytics.

Netlify's Next.js adapter requires `.next` as its publish directory to retain the build cache; it replaces that directory with the static `out` export before publishing. Local previews still serve `out`.

`pnpm-workspace.yaml` blocks reviewed native install hooks because their platform binaries are already locked dependencies. Unreviewed install hooks fail installation; review them rather than allowing fallback downloads.
